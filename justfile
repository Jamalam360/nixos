...
deploy machine:
	git add . && {{ if machine != "lyra" { "sudo" } else { "" } }} nixos-rebuild switch --accept-flake-config --flake .#{{ machine }} {{ if machine == "lyra" { "--sudo --target-host \"james@176.9.22.221\" --build-host \"james@176.9.22.221\"" } else { "" } }}
