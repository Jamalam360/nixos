...
async function update_static_site(path: string) {
	...
	const commits = await gh<unknown[]>(`https://api.github.com/repos/${owner}/${repo}/commits`);
	...
	const tarball = `https://github.com/${owner}/${repo}/archive/${newRev}.tar.gz`;
	const prefetchData = await new Deno.Command("nix-prefetch-url", { args: ["--unpack", tarball] }).output();
...
for (const site of ["./services/lyra/static/cdn.nix", "./services/lyra/static/its-clearing-up.nix", "./services/lyra/static/teach-man-fish.nix"]) {
